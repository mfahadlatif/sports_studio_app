...
class ManagedEventsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('My Managed Events'),
        centerTitle: true,
      ),
      body: Center(
  ...
  Widget _buildEventCard(BuildContext context, Event event, EventsController controller) {
    final imageUrl = UrlHelper.sanitizeUrl(event.image);
                      ...
                      PopupMenuButton<String>(
                        ...
                        onSelected: (value) {
                          if (value == 'edit') {
                            ...
                            _confirmDelete(context, event, controller);
                          }
                        },
                        itemBuilder: (context) => [
                          const PopupMenuItem(
  ...
  void _confirmDelete(BuildContext context, Event event, EventsController controller) {
    Get.defaultDialog(
      title: 'Delete Event',
      middleText: 'Are you sure you want to delete "${event.name}"? This action cannot be undone.',
      textConfirm: 'Delete',
      textCancel: 'Cancel',
      confirmTextColor: Colors.white,
      buttonColor: Colors.red,
      onConfirm: () {
        controller.deleteEvent(event.id, event.slug ?? '');
        Get.back();
      },
    );
  }

  void _showEditEventDialog(BuildContext context, Event event, EventsController controller) {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: AppSpacing.m),
              _buildField('Description', controller.eventDescriptionController, maxLines: 3),
              const SizedBox(height: AppSpacing.m),
              Row(
                children: [
                  Expanded(child: _buildField('Player Limit', controller.eventMaxParticipantsController, keyboardType: TextInputType.number)),
                  const SizedBox(width: AppSpacing.m),
                  Expanded(child: _buildField('Registration Fee', controller.eventRegistrationFeeController, keyboardType: TextInputType.number)),
                ],
              ),
              const SizedBox(height: AppSpacing.m),
              _buildField('Rules', controller.eventRulesController, maxLines: 2),
              const SizedBox(height: AppSpacing.m),
              _buildField('Safety Policy', controller.eventSafetyPolicyController, maxLines: 2),
              const SizedBox(height: AppSpacing.m),
                   ...
                   Expanded(
                     child: OutlinedButton.icon(
                       onPressed: () async {
                         ...
                         if (picked != null) controller.eventStartDate.value = picked;
                       }, 
                       icon: const Icon(Icons.calendar_today, size: 16),
                       label: Obx(() => Text(DateFormat('MMM dd').format(controller.eventStartDate.value))),
                     ),
                   ),
                     ...
                     child: OutlinedButton.icon(
                       onPressed: () async {
                         final picked = await showTimePicker(
                           context: context,
                           initialTime: TimeOfDay.fromDateTime(controller.eventStartDate.value),
                         );
                         if (picked != null) {
                           final current = controller.eventStartDate.value;
                           controller.eventStartDate.value = DateTime(current.year, current.month, current.day, picked.hour, picked.minute);
                         }
                       }, 
                       icon: const Icon(Icons.access_time, size: 16),
                       label: Obx(() => Text(DateFormat('hh:mm a').format(controller.eventStartDate.value))),
                     ),
                   ),
                ],
              ),
              ...
              Obx(() => SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: controller.isCreatingEvent.value 
                    ? null 
                    : () => controller.updateEvent(event.id, event.slug ?? ''),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppColors.primary,
                  ...
                  child: controller.isCreatingEvent.value
                    ? const AppProgressIndicator(color: Colors.white, size: 20)
                    : const Text('Update Event', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              )),
              const SizedBox(height: AppSpacing.xl),
            ],
